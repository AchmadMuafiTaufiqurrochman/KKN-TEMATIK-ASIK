            ...
            <table class="w-full">
                ...
                <tbody>
                    ...
                    <tr class="{{ $index % 2 === 0 ? 'bg-gray-50' : 'bg-white' }}">
                        <td class="px-6 py-4">
                            @if($ap->photo)
                                <img src="{{ asset('storage/' . $ap->photo) }}" alt="{{ $ap->name }}" class="h-12 w-12 object-cover rounded-full">
                            @else
                        ...
                        <td class="px-6 py-4 text-gray-600">
                            <span class="px-2 py-1 rounded-full text-xs font-semibold {{ $ap->gender === 'L' ? 'bg-blue-100 text-blue-800' : 'bg-pink-100 text-pink-800' }}">
                                {{ $ap->gender === 'L' ? 'Laki-laki' : 'Perempuan' }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex gap-2">
                                <button onclick="openEditModal({{ $ap->id }})" class="text-blue-600 hover:text-blue-800 transition-colors">
                                    <i data-lucide="edit" class="w-4 h-4"></i>
                                </button>
                                <form action="{{ route('admin.aparat.destroy', $ap) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                    @csrf
                                    @method('DELETE')
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-lg max-w-md w-full p-6">
            ...
            <form id="aparatForm" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-4">
                    <input name="name" placeholder="Nama Lengkap" required class="w-full border px-3 py-2 rounded-lg  text-gray-700">
                    <input name="nip" placeholder="NIP" required class="w-full border px-3 py-2 rounded-lg text-gray-700">
                    <input name="position" placeholder="Jabatan" required class="w-full border px-3 py-2 rounded-lg  text-gray-700">
                    <select name="gender" required class="w-full border px-3 py-2 rounded-lg text-gray-700">
                        <option value="">Pilih Jenis Kelamin</option>
                        <option value="L">Laki-laki</option>
                        <option value="P">Perempuan</option>
                    </select>
                    <input type="file" name="photo" accept="image/*" class="w-full border px-3 py-2 rounded-lg">
                </div>
...
function openEditModal(id) {
    ...
    document.getElementById('aparatForm').action = `/admin/aparat/${id}`;
