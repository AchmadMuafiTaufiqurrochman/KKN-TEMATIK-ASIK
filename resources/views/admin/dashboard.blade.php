        ...
        <div class="grid lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1">
                <div class="bg-white p-6 rounded-xl shadow-lg mb-6">
                    <h3 class="text-lg font-bold text-primary mb-4">Aksi Cepat</h3>
                    <div class="space-y-3">
                        <a href="{{ route('admin.citizens.index') }}" class="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                            <i data-lucide="users" class="w-5 h-5 text-primary"></i>
                            <span class="font-medium">Kelola Data Warga</span>
                        </a>
                        <a href="{{ route('admin.videos.index') }}" class="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                            <i data-lucide="video" class="w-5 h-5 text-primary"></i>
                            <span class="font-medium">Kelola Video</span>
                        </a>
                        <a href="{{ route('admin.locations.index') }}" class="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                            <i data-lucide="map-pin" class="w-5 h-5 text-primary"></i>
                            <span class="font-medium">Kelola Lokasi Peta</span>
                        </a>
                    </div>
                </div>
                ...
                <div class="bg-white p-6 rounded-xl shadow-lg">
                        ...
                        <div class="relative w-48 h-48">
                            ...
                            <div class="absolute inset-0 flex items-center justify-center">
                                <div class="text-center">
                                    <div class="text-2xl font-bold text-primary">{{ number_format($stats['total_villagers']) }}</div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="text-center">
                            ...
                            <div class="text-lg font-bold text-gray-900">{{ number_format($stats['male_villagers']) }}</div>
                            <div class="text-sm text-gray-600">Laki-laki</div>
                            <div class="text-xs text-gray-500">{{ number_format(($stats['male_villagers'] / $stats['total_villagers']) * 100, 1) }}%</div>
                        </div>
                        <div class="text-center">
                            <div class="w-4 h-4 bg-pink-600 rounded-full mx-auto mb-2"></div>
                            <div class="text-lg font-bold text-gray-900">{{ number_format($stats['female_villagers']) }}</div>
                            <div class="text-sm text-gray-600">Perempuan</div>
                            <div class="text-xs text-gray-500">{{ number_format(($stats['female_villagers'] / $stats['total_villagers']) * 100, 1) }}%</div>
                        </div>
                    </div>
                </div>
